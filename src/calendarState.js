...
import { startOfMonth, endOfMonth, startOfWeek, addDays, subMonths, addMonths, isSameDay, } from "date-fns";
export function useCalendarState() {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const startDate = addDays(startOfWeek(startOfMonth(currentDate), {
                    weekStartsOn: 1,
                }), -11);
                const endDate = addDays(endOfMonth(currentDate), 31);
                const iso8601Formatted = (date) => `${date.getUTCFullYear()}-${(date.getUTCMonth() + 1)
                    .toString()
                    .padStart(2, "0")}-${date
                    .getUTCDate()
                    .toString()
                    ...
                    .toString()
                    .padStart(3, "0")}Z`;
                const eventData = await getPublicEvents(iso8601Formatted(startDate), iso8601Formatted(endDate));
